...
import Streamly.External.Archive.Internal.Foreign (archive_entry_size, archive_read_data_block,
                ...
                Just e -> do
                    ...
                    let go2 :: Int64 -> IO Int64
                        go2 !pos = do
                            ...
                            if done then
                                return $ pos + fromIntegral (B.length b)
                            else
                                go2 $ pos + fromIntegral (B.length b)

                    readSz <- go2 0

                    if fromJust entrySz < 0 || fromJust entrySz /= fromIntegral readSz then
                        error "unexpected file size in archive"
                    else
                        go (totalFilesize + fromIntegral (fromJust entrySz)) (numFiles + 1)
