...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "write") == 0) {
    ...
    for (long long i = 0; i < num_files; i++) {
      ...
      // Put something interesting (not just all-zeroes) into the file. (We
      // never tested whether this is important.)
      for (long long j = 0; j < file_size; j++) {
        int8_t byte = i + j; // 0 to 255 repeated (but start at i to make files
                             // more different).
        *(file_buff + j) = byte;
      }
      ...
      la_ssize_t ret = archive_write_data(a, file_buff, file_size);
      if (ret != file_size) {
        printf("unexpected number of bytes written\n");
        if (ret < 0) {
          printf("archive_write_data returned %ld\n", ret);
    ...
    int block_size =
        4096; // Matches the blocksize we hardcoded in streamly-archive.
